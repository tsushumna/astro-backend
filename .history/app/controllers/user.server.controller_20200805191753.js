        ...
        if (err) {
            ...
            ToDo.countDocuments({ $and: query }, function(countErr, count) {
                if (countErr) {
                    ...
                    ToDo.find({ $and: query }).sort('-created').skip(pageOptions.page > 0 ? ((pageOptions.page - 1) * pageOptions.limit) : 0).limit(pageOptions.limit).exec(function(err, businessUsers) {});
