                    ...
                    if (err) {
                        ...
                        ToDo.countDocuments({ $and: query }, function(countErr, count) {
                                    if (countErr) {
                                        ...
                                        User.find({ $and: query }, 'DOB email mobile bloodGroup username personQuarantine firstName middleName lastName displayName created companies company addresses status profileImageURL languages gender imageDetectors categoryName').populate('personQuarantine').sort('-created').skip(pageOptions.page > 0 ? ((pageOptions.page - 1) * pageOptions.limit) : 0).limit(pageOptions.limit).exec(function(err, businessUsers) {
                                            if (err) {
                                                done(err, null);
                                            } else {
