'use strict';
var swisseph = require('swisseph');
var planets = require('../constants');
exports.getAllPlanetPositions = function (req, res) {
    swisseph.swe_set_ephe_path(__dirname + '/../ephe');
    var data = req.body;
    var date = new Date(data.date);
    var date = {
        year: date.getYear(),
        month: date.getMonth(),
        day: date.getDay(),
        hour: data.hours
    };
    swisseph.swe_julday(date.year, date.month, date.day, date.hour + 5.5, 2 + 256 + 65536 + 16, function (julday_ut) {
        console.log('Julian UT day for date:', julday_ut);
        var positions = [];
        swisseph.swe_houses(julday_ut, 16.05, 79.75, 'P', [], [], function (cusps) {
            console.log(cusps.house);
            res.jsonp(cusps.house);
        });
        // planets.planetNames.forEach(function (eachPlanet) {
        //     swisseph.swe_calc_ut(julday_ut, eachPlanet.flag, swisseph.SEFLG_SPEED, function (planetPosition) {
        //         planetPosition['name'] = eachPlanet.planetName;
        //         positions.push(planetPosition);
        //         console.log(positions);
        //     });
        // });
    });

};
function getPlanetPositions(julday_ut, done) {
    var positions = [];
    planets.planetNames.forEach(function (eachPlanet) {
        swisseph.swe_calc_ut(julday_ut, eachPlanet.flag, swisseph.SEFLG_SPEED, function (planetPosition) {
            planetPosition['name'] = eachPlanet.planetName;
            positions.push(planetPosition);
            console.log(positions);
        });
    });
    async.forEachSeries(planets.planetNames, function (eachPlanet, callback) {
        swisseph.swe_calc_ut(julday_ut, eachPlanet.flag, swisseph.SEFLG_SPEED, function (planetPosition) {
            planetPosition['name'] = eachPlanet.planetName;
            positions.push(planetPosition);
        });
        Appointment.findOne(eachAppointment, function (appointmentErr, appointment) {
            if (appointmentErr) {
                callback(appointmentErr);
            } else if (appointment) {
                appointment.lastUpdatedUser = loginuser._id;
                appointment.lastUpdated = Date.now();
                if (status) {
                    appointment.status = status;
                }
                if (proofs && proofs.length > 0) {
                    appointment.details.attachements = getProof(proofs, loginuser);
                }

                appointment.save(function (saveErr, saveRes) {
                    if (saveErr) {
                        callback(saveErr);
                    } else {
                        appointmentUtil.findOneQuery({ _id: saveRes._id }, function (appointmentPopulateErr, appointmentPopulate) {
                            if (appointmentPopulateErr) {
                                callback(appointmentPopulateErr);
                            } else {
                                updatedAppointments.push(appointmentPopulate);
                                callback();
                            }
                        });
                    }
                });
            } else {
                callback(new Error('Appointment not found'), null);
            }
        });
    }, function (appointmentErr) {
        if (appointmentErr) {
            done(appointmentErr, null);
        } else {
            done(null, updatedAppointments);
        }
    });
}